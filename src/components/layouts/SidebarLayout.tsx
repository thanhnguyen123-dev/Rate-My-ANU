...
export default function SidebarLayout({ children }: SidebarLayoutProps) {
  ...
  const getBreadcrumbItems = () => {
    ...
    if (pathSegments.includes('courses')) {
      // Add separator before Courses breadcrumb
      items.push(<BreadcrumbSeparator key="sep-courses" />);
      ...
      if (pathSegments.length > 1 && pathSegments[0] === 'courses' && pathSegments[1]) {
        // Add separator before course detail
        items.push(<BreadcrumbSeparator key="sep-course-detail" />);
